...
const GERMAN_TRANSLATIONS: Record<string, string>={
    ...
    'Active item breadcrumbs': 'Brotkrümelnavigation des aktiven Eintrags',
    'Clear level': 'Ebene leeren',
    'Cleared {count} values at level {level}.': '{count} Werte auf Ebene {level} gelöscht.',
    'Collapse level': 'Ebene einklappen',
    'Compact mode': 'Kompaktmodus',
    'Expand level': 'Ebene ausklappen',
    'Level actions': 'Ebenenaktionen',
    'Level {level}': 'Ebene {level}',
    'Level {level} collapsed.': 'Ebene {level} eingeklappt.',
    'Level {level} expanded.': 'Ebene {level} ausgeklappt.',
    'No selected items to show.': 'Keine ausgewählten Einträge vorhanden.',
    'Recent': 'Zuletzt',
    'Recently selected items': 'Zuletzt ausgewählte Einträge',
    'Reduce spacing and control sizes for small dashboard zones.': 'Abstände und Steuerelemente für kleine Dashboard-Bereiche verkleinern.',
    'Select level': 'Ebene auswählen',
    'Selected {count} values at level {level}.': '{count} Werte auf Ebene {level} ausgewählt.',
    'Selected only': 'Nur ausgewählte',
    'Show all': 'Alle anzeigen',
    'Show selected hierarchy items only': 'Nur ausgewählte Hierarchieeinträge anzeigen',
    'Use compact hierarchy layout': 'Kompaktes Hierarchielayout verwenden',
    ...
    'Hierarchy data could not be loaded': 'Hierarchiedaten konnten nicht geladen werden',
    ...
    ,'Add at least one other worksheet to the dashboard. The hierarchy source worksheet is intentionally excluded from filter targets.': 'Fügen Sie dem Dashboard mindestens ein weiteres Arbeitsblatt hinzu. Das Hierarchie-Quellarbeitsblatt ist absichtlich als Filterziel ausgeschlossen.'
    ,'Apply selected hierarchy values to one or more worksheets. Each target can use its own field and value mapping.': 'Wenden Sie ausgewählte Hierarchiewerte auf ein oder mehrere Arbeitsblätter an. Jedes Ziel kann ein eigenes Feld und eine eigene Wertzuordnung verwenden.'
    ,'Choose which hierarchy items a checkbox includes. Each target converts those items using its value mapping.': 'Wählen Sie, welche Hierarchieeinträge ein Kontrollkästchen einschließt. Jedes Ziel wandelt diese Einträge mit seiner Wertzuordnung um.'
    ,'Comparing source and target values…': 'Quell- und Zielwerte werden verglichen…'
    ,'Complete source validation to compare these values with the target field.': 'Schließen Sie die Quelldatenprüfung ab, um diese Werte mit dem Zielfeld zu vergleichen.'
    ,'Dashboard filtering is unavailable': 'Dashboard-Filterung ist nicht verfügbar'
    ,'Each target can receive IDs, labels, complete paths, or values from one hierarchy level.': 'Jedes Ziel kann IDs, Beschriftungen, vollständige Pfade oder Werte einer Hierarchieebene empfangen.'
    ,'Examples not found: {values}': 'Nicht gefundene Beispiele: {values}'
    ,'Full hierarchy path': 'Vollständiger Hierarchiepfad'
    ,'Hierarchy level': 'Hierarchieebene'
    ,'Level {count}': 'Ebene {count}'
    ,'Level {count}: {field}': 'Ebene {count}: {field}'
    ,'Other worksheets were found, but none exposes a string or integer field that can be filtered. Add the matching field to Detail or Filters in Tableau, then reopen configuration.': 'Andere Arbeitsblätter wurden gefunden, aber keines stellt ein filterbares Zeichenfolgen- oder Ganzzahlfeld bereit. Fügen Sie das passende Feld in Tableau zu Detail oder Filter hinzu und öffnen Sie die Konfiguration erneut.'
    ,'Specific hierarchy level': 'Bestimmte Hierarchieebene'
    ,'Tableau could not read a sample from this target field.': 'Tableau konnte aus diesem Zielfeld keine Stichprobe lesen.'
    ,'Tableau returned a limited sample, so additional matches may exist.': 'Tableau hat eine begrenzte Stichprobe zurückgegeben; weitere Treffer können vorhanden sein.'
    ,'The field whose values match the source value chosen below.': 'Das Feld, dessen Werte dem unten ausgewählten Quellwert entsprechen.'
    ,'The target worksheet is not currently available in Tableau.': 'Das Zielarbeitsblatt ist derzeit in Tableau nicht verfügbar.'
    ,'The selected mapping does not produce a value for any selectable hierarchy item.': 'Die ausgewählte Zuordnung erzeugt für keinen auswählbaren Hierarchieeintrag einen Wert.'
    ,'Apply JSON': 'JSON anwenden'
    ,'Assigned the legacy Unique ID value mapping to existing filter targets.': 'Vorhandenen Filterzielen wurde die bisherige Wertzuordnung „Eindeutige ID“ zugewiesen.'
    ,'Categorical worksheet filtering': 'Kategorische Arbeitsblattfilterung'
    ,'Configuration administration': 'Konfigurationsverwaltung'
    ,'Configuration imported. Review the mappings and validation before saving.': 'Konfiguration importiert. Prüfen Sie vor dem Speichern die Zuordnungen und die Validierung.'
    ,'Configuration JSON copied.': 'Konfigurations-JSON kopiert.'
    ,'Configuration JSON downloaded.': 'Konfigurations-JSON heruntergeladen.'
    ,'Configuration migration': 'Konfigurationsmigration'
    ,'Configuration repairs were applied for format {version}.': 'Für das Format {version} wurden Konfigurationskorrekturen angewendet.'
    ,'Converted the legacy single filter target to the multi-target format.': 'Das bisherige einzelne Filterziel wurde in das Mehrziel-Format umgewandelt.'
    ,'Copy failed: {message}': 'Kopieren fehlgeschlagen: {message}'
    ,'Copy JSON': 'JSON kopieren'
    ,'Detected Tableau API capabilities': 'Erkannte Tableau-API-Funktionen'
    ,'Diagnostics': 'Diagnose'
    ,'Download JSON': 'JSON herunterladen'
    ,'Filters applied': 'Angewendete Filter'
    ,'Hierarchy nodes': 'Hierarchieknoten'
    ,'Import accepts exported packages and legacy raw settings objects.': 'Der Import akzeptiert exportierte Pakete und bisherige reine Einstellungsobjekte.'
    ,'Import failed: {message}': 'Import fehlgeschlagen: {message}'
    ,'JSON file loaded. Choose Apply JSON to import it.': 'JSON-Datei geladen. Wählen Sie „JSON anwenden“, um sie zu importieren.'
    ,'JSON refreshed from the current configuration.': 'JSON wurde aus der aktuellen Konfiguration aktualisiert.'
    ,'Load JSON file': 'JSON-Datei laden'
    ,'Load time': 'Ladezeit'
    ,'Move settings between dashboards or keep a reviewable JSON backup. Live worksheet metadata is never exported.': 'Übertragen Sie Einstellungen zwischen Dashboards oder bewahren Sie eine prüfbare JSON-Sicherung auf. Live-Metadaten der Arbeitsblätter werden nie exportiert.'
    ,'Nodes reused': 'Wiederverwendete Knoten'
    ,'Paged summary data': 'Seitenweise Zusammenfassungsdaten'
    ,'Portable configuration JSON': 'Übertragbare Konfigurations-JSON'
    ,'Preserved the legacy parent-selection behavior for this hierarchy format.': 'Das bisherige Auswahlverhalten für übergeordnete Einträge dieses Hierarchieformats wurde beibehalten.'
    ,'Refresh JSON': 'JSON aktualisieren'
    ,'Refresh mode': 'Aktualisierungsmodus'
    ,'Removed stale dashboard metadata so Tableau can discover it again.': 'Veraltete Dashboard-Metadaten wurden entfernt, damit Tableau sie neu erkennen kann.'
    ,'Settings persistence API': 'API zum Speichern von Einstellungen'
    ,'Source data change events': 'Ereignisse bei Quelldatenänderungen'
    ,'Source mark selection API': 'API zur Markierungsauswahl in der Quelle'
    ,'Source rows': 'Quellzeilen'
    ,'Summary column metadata': 'Metadaten der Zusammenfassungsspalten'
    ,'This configuration already uses the current format ({version}).': 'Diese Konfiguration verwendet bereits das aktuelle Format ({version}).'
    ,'Updated the configuration format metadata.': 'Die Metadaten des Konfigurationsformats wurden aktualisiert.'
    ,'Upgraded configuration format {from} → {to}.': 'Konfigurationsformat {from} → {to} aktualisiert.'
    ,'Virtualized rendering': 'Virtualisierte Darstellung'
    ,'{count} ms': '{count} ms'
    ,'full': 'vollständig'
    ,'incremental': 'inkrementell'
    ,'unchanged': 'unverändert'
    ,'This field can be filtered, but Tableau does not expose sample values for it in the worksheet summary data.': 'Dieses Feld kann gefiltert werden, Tableau stellt dafür jedoch keine Stichprobenwerte in den Zusammenfassungsdaten des Arbeitsblatts bereit.'
    ,'Unique hierarchy ID': 'Eindeutige Hierarchie-ID'
    ,'Value compatibility': 'Wertkompatibilität'
    ,'Value sent to this field': 'An dieses Feld gesendeter Wert'
    ,'Visible item label': 'Sichtbare Eintragsbeschriftung'
    ,'{matchCount} of {sourceCount} mapped source values appear in the target sample ({percent}%).': '{matchCount} von {sourceCount} zugeordneten Quellwerten kommen in der Zielstichprobe vor ({percent} %).'
};
